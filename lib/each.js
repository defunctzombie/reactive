// 'each' binding
module.exports = function(el, val, model) {
    var self = this;
    ...
    var adapter = self.reactive.adapter(prop);
    var change_fn = function(value) {
        bind(value);
    };

    self.reactive.on('change ' + prop, change_fn);
    self.reactive.once('destroyed', function() {
        self.reactive.off('change' + prop, change_fn);
    });
