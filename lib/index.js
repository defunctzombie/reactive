...
function Reactive(el, obj, options) {
  ...
  // find all elements with id property set and add to nodes
  var nodes = this.el.querySelectorAll('[id]');
  for (var i=0 ; i<nodes.length ; ++i) {
    var node = nodes[i];
    this.nodes[node.getAttribute('id')] = node;
  }
}
...
Reactive.prototype.bind = function(name, fn) {
  ...
  // check top level element for attributes
  var attr = this.el.getAttribute(name);
  if (attr) {
    (new Binding(name, this, this.el, fn)).bind();
  }
